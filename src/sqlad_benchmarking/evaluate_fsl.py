...
def _run_target(
    ...
    *,
    ...
    # Tee this cell's log output to its own file under reports/ and attach it to every
    # run the sweep opens, exactly as the standard suite does for its cells. A sweep
    # covers several k, so each k's run carries the log up to the end of that k.
    ...
    with capture_cell_log(log_dir, stem) as cell_log:
        try:
            ...
            # The sweep's k runs are closed by now, so this lands on the parent run,
            # which is the only one still open to carry the traceback.
            if track:
                cell_log.upload()
            raise
...
def _run_target_tracked(
    *,
    ...
    """Body of :func:`_run_target`, run with a file handler already capturing this cell's log."""
    logger.info(
...
def evaluate_fsl(
    ...
    # are also uploaded to the runs the sweep opens as an MLflow artifact.
