"""RoBERTa-base embedding feature extractor.

Maps SQL queries to 768-d dense embeddings using the general-domain model
``roberta-base`` (mirrors the reference ``roberta.py``). The model is pretrained,
so ``fit`` is a no-op; the tokenizer/model are lazy-loaded on first use to keep
``build_extractor`` cheap and avoid the download in tests that never touch this
extractor. Disk caching is layered on externally via
...
class RobertaExtractor(BaseEstimator, TransformerMixin):
    """Sklearn transformer producing RoBERTa-base ``pooler_output`` embeddings.
    ...
    def _ensure_model(self) -> None:
        ...
        if getattr(self, "model_", None) is not None:
            return
        # Imported lazily so the transformers dependency is only needed when roberta
        # is actually used.
    ...
    def fit(self, X, y=None) -> "RobertaExtractor":  # noqa: N803
        return self
