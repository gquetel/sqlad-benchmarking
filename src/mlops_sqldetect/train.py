"""CLI to fit a Li SQL injection detection pipeline (OCSVM or AutoEncoder).
...
def train(
    ...
    """Fit a Li pipeline on the normal-class samples of ``dataset``."""
    ...
    model.fit(df_normal)
    model.save(output)
    logger.info(f"Saved {pipeline} pipeline to {output}")
