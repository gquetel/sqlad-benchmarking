...
class SecureBertExtractor(BaseEstimator, TransformerMixin):
    ...
    def _ensure_model(self) -> None:
        ...
        if getattr(self, "model_", None) is not None:
            return
        # Imported lazily so the transformers dependency is only needed when sbert
        # is actually used.
    ...
    def fit(self, X, y=None) -> "SecureBertExtractor":  # noqa: N803
        return self
