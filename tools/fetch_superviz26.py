"""Download the Superviz26-SQL datasets from Zenodo with checksum verification.

Record 21068333 ships every Superviz26 build inside a single ``superviz26.zip``
archive. This tool downloads that archive once (MD5-verified, HTTP-Range resume),
extracts the requested group(s) into their loader default roots, and verifies each
extracted CSV's size and SHA-256 against ``MANIFEST.json``. The groups are:

  * ``main``  -> the 8 LODO/in-domain scenario CSVs (``~/datasets/superviz26-lodo/``);
  * ``drift`` -> the per-domain concept-drift CSVs (``~/datasets/superviz26-cd/``);
  * ``fsl``   -> the per-target-domain few-shot CSVs (``~/datasets/superviz26-fsl/``).

Usage:
    python -m tools.fetch_superviz26 --groups main          # default superviz26 data
    python -m tools.fetch_superviz26 --groups drift,fsl     # heavy supplementary data
    python -m tools.fetch_superviz26                        # every group in one pass
"""
...
MANIFEST_PATH = REPO_ROOT / "data" / "raw" / "superviz26" / "MANIFEST.json"
CHUNK_BYTES = 1 << 20
...
GROUP_ROOTS: dict[str, Callable[[], Path]] = {
    "main": superviz26.default_root,
    "drift": superviz26_drift.default_root,
    "fsl": superviz26_fsl.default_root,
}
...
@dataclass(frozen=True)
class Archive:
    """The single Zenodo zip that bundles every Superviz26 group."""
...
def fetch(
    groups: Annotated[
        str,
        typer.Option(help="Comma-separated groups to fetch ('main', 'drift', 'fsl'). Omit for all."),
    ] = "",
    ...
    archive_dir: Annotated[
        Path | None,
        typer.Option(help="Where to cache the downloaded zip (default: parent of the data roots, i.e. ~/datasets)."),
    ] = None,
) -> None:
    """Download (or verify) the Superviz26 CSVs from the single Zenodo archive."""
    ...
    archive_path = (archive_dir or GROUP_ROOTS["main"]().parent) / archive.name
    failures = _extract_pending(archive, pending, archive_path=archive_path, force=force, keep_archive=keep_archive)
    if failures:
        logger.error(f"{len(failures)} file(s) failed: {failures}")
        sys.exit(1)
