...
def run_cell(
    ...
    # Open the figure browser here, before the cell loads its data: starting it later,
    # from a process already holding the embeddings and the test sets, is what makes it
    # die on a compute node.
    with image_export():
...
def _dispatch(
    ...
    """Run one cell through the evaluator its dataset family calls for.

    Concept-drift cells train once and evaluate two test sets, few-shot cells adapt a
    pretrained LODO model over a k-sweep, so each runs through its own evaluator
    instead of the standard suite.
    """
